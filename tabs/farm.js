(function(module) {
            ...
            <div class="bot-section">
                ...
                <div class="section-content">
                    ...
                    <div style="margin-top:10px;padding:10px;background:rgba(0,0,0,0.2);border-radius:6px;font-size:11px;color:#BDB76B;">
                        ℹ️ 2 actions séquentielles (fermées obligatoirement), puis récolte sécurisée, puis délai aléatoire selon l'option de pillage.
                    </div>
                </div>
            </div>
    ...
    function createStopButton() {
        ...
        overlay.innerHTML = `
            <button class="gu-stop-routine-btn" id="gu-stop-action-btn">
    ...
    module.init = function() {
        ...
        document.getElementById('toggle-farm').onchange = (e) => {
            toggleFarm(e.target.checked);
        };
        ...
        document.getElementById('farm-mode').onchange = (e) => {
            ...
            log(
                'FARM',
                'Mode: ' + (
                    e.target.value === 'least_resources'
                        ? 'Villes vides'
                        : 'Cyclique'
                ),
                'info'
        ...
        document.getElementById('farm-duration').onchange = (e) => {
            ...
            log(
                'FARM',
                `Option de pillage configurée : ${opt.label}`,
                'info'
            );
        };
    ...
    function emergencyStop() {
        ...
        if (status) {
            status.textContent = 'Arrêt d\'urgence';
        }
        ...
        log(
            'FARM',
            '🚨 STOP CURRENT ROUTINE : Routine stoppée par l\'utilisateur.',
            'error'
        );
    ...
    // ─── DÉLAI SELON L'OPTION DE PILLAGE ──────────────────────────────────────────
    //
    // 5 minutes  → délai aléatoire entre 5 et 7 minutes
    // 10 minutes → délai aléatoire entre 10 et 17 minutes
    // 20 minutes → comportement de sécurité : 10 à 17 minutes
    ...
    function getRandomIntervalMs() {
        ...
        if (farmData.settings.duration === 1) {

            // Option de pillage : 5 minutes
            minSec = 5 * 60;
            maxSec = 7 * 60;

        } else if (farmData.settings.duration === 2) {

            // Option de pillage : 10 minutes
            minSec = 10 * 60;
            maxSec = 17 * 60;
            ...
            // Option 20 minutes :
            // aucun comportement spécifique demandé,
            // on conserve le délai 10-17 minutes.
            minSec = 10 * 60;
            maxSec = 17 * 60;
        }
        ...
        const randomSecs =
            Math.floor(
                Math.random() * (maxSec - minSec + 1)
            ) + minSec;

        return randomSecs * 1000;
    }

    // ─── 2 ACTIONS SÉQUENTIELLES STRICTES ─────────────────────────────────────────
    ...
    async function performRandomHumanActions() {
        ...
        try {
            ...
            log(
                'FARM',
                'Routine humaine : Exécution des 2 actions aléatoires séquentielles...',
                'info'
            );
            ...
            const possibleTasks = [

                {
                    ...
                    type:
                        uw.GPWindowMgr?.TYPE_MESSAGE ||
                        uw.GPWindowMgr?.TYPE_MAIL,
...

                {
                    ...
                    type:
                        uw.GPWindowMgr?.TYPE_ALLIANCE_FORUM ||
                        uw.GPWindowMgr?.TYPE_FORUM,
                    ...
                    action: async (win) => {
                        ...
                        if (winEl) {
                            ...
                            const tabs = $(winEl).find(
                                '.forum_tabs a, ' +
                                '.sub_tabs a, ' +
                                '.forum_navigation li a, ' +
                                '.nui_tabs_container a, ' +
                                '.tabs_container a'
                            );
            ...
            for (const task of selectedTasks) {
                ...
                log(
                    'FARM',
                    `Ouverture de l'onglet : [${task.name}]`,
                    'info'
                );
                ...
                try {
                    ...
                    if (
                        uw.GPWindowMgr &&
                        typeof uw.GPWindowMgr.Create === 'function'
                    ) {
                    ...
                    log(
                        'FARM',
                        `Fermeture obligatoire de l'onglet : [${task.name}]`,
                        'info'
                    );
                    ...
                    try {
                        ...
                        if (
                            winInstance &&
                            typeof winInstance.close === 'function'
                        ) {
                            winInstance.close();
                        }
                    ...
                    if (
                        uw.GPWindowMgr &&
                        typeof uw.GPWindowMgr.CloseAllOpenWindowsOfType === 'function'
                    ) {

                        try {

                            uw.GPWindowMgr.CloseAllOpenWindowsOfType(
                                task.type
                            );
    ...
    async function runFarmCycle() {
        ...
        // 3. Programmer le prochain passage
        const nextDelay =
            getRandomIntervalMs();
        ...
        log(
            'FARM',
            `Prochain cycle planifié dans ~${mins}m ${secs}s.`,
            'info'
        );
    ...
    async function executeFarmClaim() {
        ...
        try {
            ...
            if (
                farmData.settings.mode === 'round_robin'
            ) {
                ...
                uw.gpAjax.ajaxPost(
                    ...
                    (resp) => {
                        ...
                        if (realGain === 0) {
                            ...
                            log(
                                'FARM',
                                `⚠️ ${ids.length} île(s) — villages vides (0 res)`,
                                'warning'
                            );
                            ...
                            log(
                                'FARM',
                                `✅ ${ids.length} île(s) récoltée(s), +${displayGain} res`,
                                'success'
                            );

                            sendWebhook(
                                'Récolte Auto Farm (Sécurisé)',
    ...
    function getPolisList() {
        ...
        for (const t of towns) {
            ...
            const islandId =
                t.attributes.island_id;

            const res =
                t.attributes.resources ||
                {};
            ...
            if (islandMap.has(islandId)) {
                ...
                if (
                    farmData.settings.mode === 'least_resources' &&
                    townData.total <
                    islandMap.get(islandId).total
                ) {
    ...
    function updateStats() {
        ...
        if (r) {
            r.textContent =
                farmData.stats.totalRes.toLocaleString();
        }
    }
            ...
            data:
                JSON.stringify({
                    embeds: [{
                        ...
                        footer: {
                            text:
                                'Grepolis Ultimate — Auto Farm (Humain)'
                        },
                        timestamp:
